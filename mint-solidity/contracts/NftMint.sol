...
contract NftMint is ERC721URIStorage, ERC2981, Ownable, ReentrancyGuard, Pausable, NFTProxy {
    ...
    uint64 private _preCost = 0.01 ether; // uint64 (0 ~ 18,446,744,073,709,551,615)
    uint64 private _pubCost = 0.02 ether; // uint64 (0 ~ 18,446,744,073,709,551,615)
    ...
    function teamMint(uint8 mintAmount_, address recipient) external onlyOwner nonReentrant payable {
        mint(mintAmount_, recipient);
    }

    function freeMint(uint8 mintAmount_, bytes32[] calldata proof_, address recipient) external nonReentrant payable {
    ...
    function preMint(uint8 mintAmount_, bytes32[] calldata proof_, address recipient) external nonReentrant payable {
    ...
    function publicMint(uint8 mintAmount_, address recipient) external nonReentrant payable {
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public onlyApprovedMarketplace virtual override {
